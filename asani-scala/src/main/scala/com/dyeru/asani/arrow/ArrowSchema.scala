...
object ArrowSchema {
  inline def derived[T](using p: Mirror.Of[T]): ArrowSchema[T] = new ArrowSchema[T] {
...
//trait ArrowSchema[A] {
//
//  inline def schemaOf(using p: Mirror.ProductOf[A]): Schema = {
//    val labels = constValueTuple[p.MirroredElemLabels].toList.asInstanceOf[List[String]]
//    val types = getTypes[p.MirroredElemTypes]
//
//    val arrowFields = labels.zip(types)
...
//    }
//
//  inline given derivedArrowSchema[T](using m: Mirror.ProductOf[T]): Schema = ArrowSchema.schemaOf[T]
//}
//
